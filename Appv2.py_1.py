# app.py — STM Dashboard (GeoNet + IRIS) + USGS + STI Heatmap + Alerts (fixed tz)
# --------------------------------------------------------------------------------
...
with st.sidebar:
    ...
    iris_cha = st.text_input("cha", value="BDF")
    st.caption("If channel has no data, fallback order: BDF → LDF → HDH.")
