...
# ======================================================
# Simulated feeds (used until CSVs uploaded or real endpoints provided)
# ======================================================
@st.cache_data(ttl=300)
def sim_gps(last_hours=168):
...
@st.cache_data(ttl=300)
def sim_infrasound(last_hours=168):
    t = pd.date_range(datetime.now(TZ) - timedelta(hours=last_hours), datetime.now(TZ), freq="30min")
...
def compute_sti(gps_df: pd.DataFrame, infra_df: pd.DataFrame, beta: float = 0.05, phi_window_hours:int=24):
    ...
    w = max(2, int(phi_window_hours*2))  # 30min sampling → hours*2
...
def forecast_7d_sti(last_value: float):
    """Simple damped oscillation forecast (transparent); 7 days hourly."""
    horizon = 7*24
...
# ======================================================
# Data source precedence: session_state (uploads) → simulation
# ======================================================
if "gps_df" in st.session_state and isinstance(st.session_state["gps_df"], pd.DataFrame):
    gps = st.session_state["gps_df"]
else:
    gps = sim_gps(168)

if "infra_df" in st.session_state and isinstance(st.session_state["infra_df"], pd.DataFrame):
    infra = st.session_state["infra_df"]
else:
    infra = sim_infrasound(168)
...
with tab_dash:
    ...
    if df.empty:
        st.warning("No data yet. Upload CSVs in the Data tab or use simulation.")
    else:
        ...
        phi_now = df["phi"].iloc[-1] if "phi" in df.columns else np.nan
        grad_phi_now = df["grad_phi"].iloc[-1] if "grad_phi" in df.columns else np.nan
    ...
    if not gps.empty and not infra.empty:
        ...
        if not joined.empty:
            ...
            try:
                ...
                pass
            fig_sc.update_layout(margin=dict(l=10,r=10,t=10,b=10), height=360,
                                 xaxis_title="∇φ (GPS-derived)", yaxis_title="Infrasound amplitude")
...
with tab_data:
    ...
    if up_gps is not None:
        try:
            ...
            gdf = gdf.dropna(subset=["time","displacement"]).sort_values("time").reset_index(drop=True)
            st.session_state["gps_df"] = gdf
    ...
    if up_inf is not None:
        try:
            ...
            idf = idf.dropna(subset=["time","infrasound"]).sort_values("time").reset_index(drop=True)
            st.session_state["infra_df"] = idf
            st.success("Infrasound loaded.")
            st.experimental_rerun()
        except Exception as e:
            st.error(f"Infrasound load error: {e}")

    # Show what app currently uses
    current_gps = st.session_state.get("gps_df", gps)
    current_inf = st.session_state.get("infra_df", infra)

    st.markdown("### Preview (tail)")
    d1, d2 = st.columns(2)
    d1.dataframe(current_gps.tail(20), use_container_width=True)
    d2.dataframe(current_inf.tail(20), use_container_width=True)

    joined_view = compute_sti(current_gps, current_inf, beta=beta, phi_window_hours=phi_win_h).tail(30)
    st.markdown("### Joined STI (tail)")
    st.dataframe(joined_view, use_container_width=True)
...
with tab_echo:
    ...
    st.markdown("> Tip: Upload CSVs with UTC timestamps like 2025-09-10T00:00:00Z. App converts to Europe/Berlin internally.")