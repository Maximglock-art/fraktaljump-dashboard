# app.py — STM Dashboard with REAL data fetchers (GeoNet + IRIS) + USGS Map
# ------------------------------------------------------------------------
...
def fetch_geonet_gps(site="ANAU", hours=48):
    ...
    Документация: fits.geonet.org.nz (type=geojson)
    """
    end = datetime.now(TZ)
...
def fetch_iris_infrasound(net="IU", sta="ANMO", loc="00", cha="BDF", hours=48):
    ...
    Не все станции имеют инфразвук (BDF/LDF/HDH и т.п.).
    """
    end = datetime.now(TZ)
    start = end - timedelta(hours=hours)
    url = (
...
def fetch_iris_station_coords(net: str, sta: str):
    """
    Координаты станции IRIS через FDSN Station (text).
    ...
    try:
        ...
        return None, None
    # формат: строки, где столбцы через |, есть заголовки # ...
    for line in txt.splitlines():
        if not line or line.startswith("#"):
            continue
        parts = [p.strip() for p in line.split("|")]
        # классический порядок: Network | Station | Latitude | Longitude | Elevation | ...
        if len(parts) >= 4:
            try:
...
def fetch_usgs_quakes(hours: int = 24, minmag: float = 2.5):
    """
    Свежие землетрясения USGS (GeoJSON). Возвращает DataFrame: time, mag, place, lat, lon, depth_km
    """
    try:
...
with tab_map:
    st.subheader("Map — USGS quakes + station")
    col1, col2, col3, col4 = st.columns([1,1,1,1.5])
    show_quakes = col1.toggle("USGS quakes", True)
    q_hours     = col2.slider("Окно (ч)", 1, 72, 24, 1)
    q_minmag    = col3.slider("Min M", 0.0, 7.0, 2.5, 0.1)
    show_sta    = col4.toggle("Показать станцию IRIS", True)

    # базовая пустая карта
    figm = go.Figure()
    figm.update_layout(
    ...
    if show_sta:
        ...
        if lat_sta is not None and lon_sta is not None:
            figm.add_trace(go.Scattermapbox(
                lat=[lat_sta], lon=[lon_sta],
                mode="markers+text",
                marker=dict(size=14),
                text=[f"{iris_net}.{iris_sta} {iris_cha}"],
    ...
    if show_quakes:
        ...
        if not qdf.empty:
            ...
            st.caption("USGS: нет событий под выбранные фильтры.")
